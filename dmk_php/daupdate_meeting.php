...
if ($result = mysqli_query($mysqli, $sql1)) {
   ...
   if ($meeting_participants == 2) {
    $sql = "SELECT whatsapp_no from user_master where category ='OB' AND district= $meeting_constituency";
    if ($result = mysqli_query($mysqli, $sql)) {
        ...
        $sql = "SELECT whatsapp_no from user_master where category ='EN' AND district= $meeting_constituency";
    if ($result = mysqli_query($mysqli, $sql)) {
        $cr = 0;
        while ($row = mysqli_fetch_assoc($result)) {
            $user_email[$cr]['whatsapp_no'] = $row['whatsapp_no'];
